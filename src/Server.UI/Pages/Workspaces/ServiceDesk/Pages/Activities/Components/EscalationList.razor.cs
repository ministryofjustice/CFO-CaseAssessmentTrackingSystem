...
public partial class EscalationList
{
    ...
    private void ViewActivity(ActivityQueueEntryDto dto) => Navigation.NavigateTo($"/pages/workspace/servicedesk/activities/escalation/{dto.Id}?from=activities-queue&tab=escalation");

    private void ViewParticipant(ActivityQueueEntryDto dto) => Navigation.NavigateTo($"/pages/workspace/participants/{dto.ParticipantId}?from=activities-queue&tab=escalation");
