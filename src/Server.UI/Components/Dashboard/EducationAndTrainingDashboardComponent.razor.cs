...
public partial class EducationAndTrainingDashboardComponent
{
    ...
    private ApexChartOptions<GetEducationAndTrainings.LocationDetail> Options => new()
    {
        Chart = new Chart
        {
            Stacked = true
        ...
        [
            ...
            {
                Breakpoint = 768,
                Options = new ApexChartOptions<GetEducationAndTrainings.LocationDetail>
                {
                    Legend = new Legend
                    {
    ...
    private async Task OnExport()
    {
        ...
        {
            _downloading = true;
            var result = await Service.Send(new ExportEducationAndTraining.Command()
            {
                Request = new ExportEducationAndTraining.EducationAndTrainingExportRequest
                {
                    StartDate = DateRange?.Start ?? throw new InvalidOperationException("DateRange not set"),
                    EndDate = DateRange?.End ?? throw new InvalidOperationException("DateRange not set"),
                    TenantId = TenantId,
