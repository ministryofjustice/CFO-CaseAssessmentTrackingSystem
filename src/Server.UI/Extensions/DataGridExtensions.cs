        ...
        foreach (var sortDefinition in sortDefinitions)
        {
            ...
            if (orderedQuery is null)
            {
                var sortMethod = typeof(Queryable)
                    .GetMethods()
                    .Where(m =>
                        m.Name == (sortDefinition.Descending ? "OrderByDescending" : "OrderBy")
                ...
                orderedQuery = (IOrderedQueryable<T>?)
                    genericMethod.Invoke(genericMethod, new object[] { source, sortLambda });
            }
            else
            {
                var sortMethod = typeof(Queryable)
                    .GetMethods()
                    .Where(m =>
                        m.Name == (sortDefinition.Descending ? "ThenByDescending" : "ThenBy")
                        && m.IsGenericMethodDefinition
                    ) // ensure selecting the right overload
                    .Single(m => m.GetParameters().ToList().Count == 2);
                var genericMethod = sortMethod.MakeGenericMethod(typeof(T), orderByProperty.Type);
