...
/// <summary>
/// Scopes the user picker on the Performance / Education &amp; Training tab to support workers who
/// own an activity with an eligible education payment in the tenant scope, mirroring
/// <see cref="GetEducationAndTrainings"/>.
/// </summary>
public static class GetEducationAndTrainingAssignees
{
    ...
    public class Handler(IUnitOfWork unitOfWork) : IQueryHandler<Query, Result<AssigneeDto[]>>
    {
        public async Task<Result<AssigneeDto[]>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var assignees = await (
                    from mi in context.EducationPayments
