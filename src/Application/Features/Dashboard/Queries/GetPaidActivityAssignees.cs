...
/// <summary>
/// Scopes the user picker on the Performance / Activities (paid activities) tab to support workers
/// who own an activity with an eligible payment in the tenant scope, mirroring <see cref="GetPaidActivities"/>.
/// </summary>
public static class GetPaidActivityAssignees
{
    ...
    public class Handler(IUnitOfWork unitOfWork) : IQueryHandler<Query, Result<AssigneeDto[]>>
    {
        public async Task<Result<AssigneeDto[]>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var assignees = await (
                    from mi in context.ActivityPayments
