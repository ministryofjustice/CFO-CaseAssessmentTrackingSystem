...
public static class ExportActivitiesAdvisoriesToProvider
{
    [RequestAuthorize(Policy = SecurityPolicies.Internal)]
    public class Command : IRequest<Result>
    {
        public required GetActivitiesAdvisoriesToProvider.Query Query { get; set; }
    }
        ...
        ICurrentUserService currentUser) : IRequestHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var document = GeneratedDocument
                .Create(DocumentTemplate.ActivitiesAdvisoriesToProvider, "ActivitiesAdvisoriesToProvider.xlsx", "Activities Advisories To Provider Export", currentUser.UserId!, currentUser.TenantId!, json);
