...
public static class ExportEnrolmentAdvisoriesToProvider
{
    [RequestAuthorize(Policy = SecurityPolicies.Internal)]
    public class Command : IRequest<Result>
    {
        public required GetEnrolmentAdvisoriesToProvider.Query Query { get; set; }
    }
        ...
        ICurrentUserService currentUser) : IRequestHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var document = GeneratedDocument
                .Create(DocumentTemplate.EnrolmentAdvisoriesToProvider, "EnrolmentAdvisoriesToProvider.xlsx", "Enrolment Advisories To Provider Export", currentUser.UserId!, currentUser.TenantId!, json);
