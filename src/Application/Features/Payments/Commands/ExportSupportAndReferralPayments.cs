...
public static class ExportSupportAndReferralPayments
{
    [RequestAuthorize(Roles = $"{RoleNames.SystemSupport}, {RoleNames.Finance}")]
    public class Command : ICommand<Result>
    {
        public required GetSupportAndReferralPayments.Query Query { get; set; }
    }
        ...
        IContractService contractService) : ICommandHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var filename = ExportDocumentNaming.BuildFileName("SupportAndReferralPayments");
            ...
            var description = ExportDocumentNaming.BuildDescription(
                "SupportAndReferralPayments Export",
                ("Contract", contractName),
                ("Date", new DateTime(request.Query.Year, request.Query.Month, 1).ToString("MMM yyyy")));

            var document = GeneratedDocument
                .Create(DocumentTemplate.SupportAndReferralPayments, filename, description, currentUser.UserId!, currentUser.TenantId!, json);
