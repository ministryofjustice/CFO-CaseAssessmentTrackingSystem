...
public class DocumentExportPerformanceReassessmentsIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceReassessmentsIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceReassessments.Name)
        {
        ...
        {
            logger.LogError(
                "Performance reassessments export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportReassessments.ReassessmentsExportRequest>(context.SearchCriteria!)
            ...
            {
                logger.LogError("Failed to upload performance reassessments document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance reassessments document {DocumentId}: {ErrorMessage}",
