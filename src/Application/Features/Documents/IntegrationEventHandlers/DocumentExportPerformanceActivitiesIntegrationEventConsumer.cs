...
public class DocumentExportPerformanceActivitiesIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceActivitiesIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceActivities.Name)
        {
        ...
        {
            logger.LogError(
                "Performance activities export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportActivities.ActivitiesExportRequest>(context.SearchCriteria!)
            ...
            {
                logger.LogError("Failed to upload performance activities document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance activities document {DocumentId}: {ErrorMessage}",
