...
public class DocumentExportPerformanceEnrolmentsIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceEnrolmentsIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceEnrolments.Name)
        {
        ...
        {
            logger.LogError(
                "Performance enrolments export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportEnrolments.EnrolmentsExportRequest>(context.SearchCriteria!)
            ...
            {
                logger.LogError("Failed to upload performance enrolments document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance enrolments document {DocumentId}: {ErrorMessage}",
