...
public class DocumentExportPerformanceEducationAndTrainingIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceEducationAndTrainingIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceEducationAndTraining.Name)
        {
        ...
        {
            logger.LogError(
                "Performance education and training export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportEducationAndTraining.EducationAndTrainingExportRequest>(
            ...
            {
                logger.LogError("Failed to upload performace education & training document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex,
                "Error exporting performance education & training document {DocumentId}: {ErrorMessage}",
