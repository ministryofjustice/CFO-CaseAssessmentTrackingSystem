...
public class DocumentExportEnrolmentPaymentsSummaryIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ITargetsProvider targetsProvider,
    ILogger<DocumentExportEnrolmentPaymentsSummaryIntegrationEventConsumer> logger) : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.EnrolmentPaymentsSummary.Name)
        {
        ...
        if (document is null)
        {
            logger.LogError("Export enrolment payments summary document event raised for a document that does not exist. ({DocumentId})", context.DocumentId);
            return;
        }
        ...
        try
        {
            ...
            var results = await excelService.ExportAsync(data.Data?.ContractSummary ?? [],
                new Dictionary<string, Func<EnrolmentPaymentSummaryDto, object?>>
                {
                    { "Contract", item => item.Contract },
                    { "Custody Ach", item => item.Custody },
                    { "Custody Tgt", item => item.CustodyTarget },
                    { "Custody %", item => item.CustodyPercentage },
                    { "Community Ach", item => item.Community },
                    { "Community Tgt", item => item.CommunityTarget },
                    { "Community %", item => item.CommunityPercentage }
                }
            );
            ...
            {
                logger.LogError("Failed to upload enrolment payments summary document {DocumentId}: {Errors}", context.DocumentId, string.Join(", ", result.Errors));
        ...
        {
            logger.LogError(ex, "Error exporting enrolment payments summary document {DocumentId}: {ErrorMessage}", context.DocumentId, ex.Message);
            document.WithStatus(DocumentStatus.Error);
            await unitOfWork.CommitTransactionAsync();
        }
    }
