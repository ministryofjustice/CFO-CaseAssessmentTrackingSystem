    ...
    ITargetsProvider targetsProvider) : IConsumer<ExportDocumentIntegrationEvent>
{
    ...
    {
        if (context.Message.Key != DocumentTemplate.SupportAndReferralPayments.Name)
        {
            return;
        }
        ...
        try
        {
            var request = JsonConvert.DeserializeObject<GetSupportAndReferralPayments.Query>(context.Message.SearchCriteria!)
                ?? throw new Exception();
