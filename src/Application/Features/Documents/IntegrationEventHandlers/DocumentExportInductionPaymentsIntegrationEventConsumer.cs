...
public class DocumentExportInductionPaymentsIntegrationEventConsumer(
    ...
    ILogger<DocumentExportInductionPaymentsIntegrationEventConsumer> logger) : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        ...
        if (document is null)
        {
            logger.LogError("Export induction payments document event raised for a document that does not exist. ({DocumentId})", context.DocumentId);
            return;
        }
        ...
        try
        {
            ...
            {
                logger.LogError("Failed to upload induction payments document {DocumentId}: {Errors}", context.DocumentId, string.Join(", ", result.Errors));
        ...
        {
            logger.LogError(ex, "Error exporting induction payments document {DocumentId}: {ErrorMessage}", context.DocumentId, ex.Message);
            document.WithStatus(DocumentStatus.Error);
            await unitOfWork.CommitTransactionAsync();
        }
    }
