    ...
    ITargetsProvider targetsProvider) : IConsumer<ExportDocumentIntegrationEvent>
{
    ...
    {
        if (context.Message.Key != DocumentTemplate.InductionPayments.Name)
        {
            return;
        }
        ...
        try
        {
            var request = JsonConvert.DeserializeObject<GetInductionPayments.Query>(context.Message.SearchCriteria!)
                ?? throw new Exception();
