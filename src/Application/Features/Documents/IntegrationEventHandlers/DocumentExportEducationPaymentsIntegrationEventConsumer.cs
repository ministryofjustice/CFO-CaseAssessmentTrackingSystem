    ...
    ITargetsProvider targetsProvider) : IConsumer<ExportDocumentIntegrationEvent>
{
    ...
    {
        if (context.Message.Key != DocumentTemplate.EducationPayments.Name)
        {
            return;
        }
        ...
        try
        {
            var request = JsonConvert.DeserializeObject<GetEducationPayments.Query>(context.Message.SearchCriteria!)
                ?? throw new Exception();
