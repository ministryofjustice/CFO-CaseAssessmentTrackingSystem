...
public class DocumentExportPerformanceSupportAndReferralIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceSupportAndReferralIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceSupportAndReferral.Name)
        {
        ...
        {
            logger.LogError(
                "Performance support and referral export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        try
        {
            var request =
                JsonConvert
                    .DeserializeObject<ExportSupportAndReferral.SupportAndReferralExportRequest>(
            ...
            {
                logger.LogError("Failed to upload performance support & referral document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance support & referral document {DocumentId}: {ErrorMessage}",
