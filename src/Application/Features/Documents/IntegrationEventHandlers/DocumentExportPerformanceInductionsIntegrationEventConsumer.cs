...
public class DocumentExportPerformanceInductionsIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceInductionsIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceInductions.Name)
        {
        ...
        {
            logger.LogError(
                "Performance inductions export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportInductions.InductionsExportRequest>(context.SearchCriteria!)
            ...
            {
                logger.LogError("Failed to upload performance inductions document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance inductions document {DocumentId}: {ErrorMessage}",
