...
public class DocumentExportPerformanceEmploymentsIntegrationEventConsumer(
    IUnitOfWork unitOfWork,
    IExcelService excelService,
    IUploadService uploadService,
    IDomainEventDispatcher domainEventDispatcher,
    ILogger<DocumentExportPerformanceEmploymentsIntegrationEventConsumer> logger)
    : IHandleMessages<ExportDocumentIntegrationEvent>
{
    public async Task Handle(ExportDocumentIntegrationEvent context)
    {
        if (context.Key != DocumentTemplate.PerformanceEmployments.Name)
        {
        ...
        {
            logger.LogError(
                "Performance employments export event raised for a document that does not exist. ({DocumentId})",
                context.DocumentId);
            return;
        }
        ...
        {
            var request =
                JsonConvert.DeserializeObject<ExportEmployments.EmploymentExportRequest>(context.SearchCriteria!)
            ...
            {
                logger.LogError("Failed to upload performance employments document {DocumentId}: {Errors}",
        ...
        {
            logger.LogError(ex, "Error exporting performance employments document {DocumentId}: {ErrorMessage}",
